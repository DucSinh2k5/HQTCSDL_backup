...
class EvaluateModelTests(unittest.TestCase):
    ...
    def test_evaluate_model_assigns_calibrated_return_series_positionally(self):
        test_df = pd.DataFrame(
            {
                "close": [100.0, 100.0, 100.0],
                "future_close": [110.0, 95.0, 100.5],
                "target_close": [110.0, 95.0, 100.5],
                "target_return": [0.10, -0.05, 0.005],
            },
            index=[1, 2, 0],
        )
        X_test = pd.DataFrame({"feature": [1.0, 2.0, 3.0]})
        ...
        class SeriesCalibrator:
            def predict(self, raw_returns):
                return pd.Series([0.10, -0.05, 0.005])
        ...
        self.assertFalse(result_df["predicted_return"].isna().any())
        self.assertFalse(result_df["predicted_close"].isna().any())
        np.testing.assert_allclose([0.10, -0.05, 0.005], result_df["predicted_return"])
        np.testing.assert_allclose([110.0, 95.0, 100.5], result_df["predicted_close"])
        self.assertAlmostEqual(0.0, metrics["Return_MAE"])

    def test_evaluate_model_rejects_mismatched_calibrated_return_length(self):
        test_df = pd.DataFrame(
            {
                "close": [100.0, 100.0, 100.0],
                "future_close": [110.0, 95.0, 100.5],
        ...
        class ShortCalibrator:
            def predict(self, raw_returns):
                return pd.Series([0.10, -0.05])

        with self.assertRaisesRegex(
            ValueError, "predicted return length must match test rows"
        ):
            evaluate_model(
                model=model,
                X_test=X_test,

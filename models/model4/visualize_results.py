...
# Hỗ trợ tiếng Việt trong biểu đồ
...
# Feature Importance
# ==================
print("[model4] Vẽ biểu đồ Feature Importance...")
...
# Thêm metrics vào góc
textstr = (f"Accuracy:  {metrics['accuracy']:.4f}\n"
           f"Precision: {metrics['precision']:.4f}\n"
           f"Recall:    {metrics['recall']:.4f}\n"
           f"F1-score:  {metrics['f1']:.4f}")
ax.text(1.35, 0.5, textstr, transform=ax.transAxes,
        fontsize=10, va="center",
        bbox=dict(boxstyle="round", facecolor="lightyellow", alpha=0.8))

plt.tight_layout()
plt.savefig(CHARTS_DIR / "2_confusion_matrix.png",
...
# Phân phối xác suất
# ==================
print("[model4] Vẽ phân phối xác suất dự đoán...")

fig, ax = plt.subplots(figsize=(9, 6))

# Vẽ histogram cho từng nhóm
prob_outperform     = df[df["label"] == 1]["outperform_probability"]
prob_not_outperform = df[df["label"] == 0]["outperform_probability"]

ax.hist(prob_not_outperform, bins=50, alpha=0.6,
        color="tomato", label="Thực tế: Không Outperform (0)",
        density=True)
ax.hist(prob_outperform, bins=50, alpha=0.6,
        color="steelblue", label="Thực tế: Outperform (1)",
        density=True)

ax.axvline(x=0.5, color="black", linestyle="--",
           lw=1.5, label="Ngưỡng = 0.5")
